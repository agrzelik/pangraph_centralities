...
def get_pangraph_vertices(pangraph_levi_adjacency):
...
def df_operations(df, to_add, to_del):
    ...
    dfpangraph_edges = df.copy(deep=True)
     
    dfpangraph_edges = dfpangraph_edges.drop(to_del)
...
def test_degrees():
    ...
    df_code = pd.read_csv('test_output_files/degrees.csv', sep = ';')
...
for test_run_on in [True, False]: 
    ...
    h_df.columns = ['in', 'out']
    p_levi_df = pd.read_csv(base_path+'levi_pangraph_edges_list.csv',index_col=0, delimiter = ';')
    p_levi_df.columns = [ 'in', 'out']
